...
@group(#{MATERIAL_BIND_GROUP}) @binding(3) var<uniform> screen_offset: vec2<f32>;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let uv = in.uv - screen_offset;
    let sample = textureSample(source, source_sampler, uv).rgb;
    let center = select(sample, vec3<f32>(0.0), any(uv < vec2<f32>(0.0)) || any(uv > vec2<f32>(1.0)));
