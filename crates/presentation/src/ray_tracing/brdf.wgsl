...
// Licensed under MIT OR Apache-2.0. Zero-reflectance nonmetals use a
// diffuse-only response for painted classroom characters. Other materials
// retain the upstream BRDF.
...
fn evaluate_and_sample_brdf(
    ...
    let matte = material.reflectance == 0.0 && material.metallic == 0.0;
    let diffuse_weight = select(mix(df, 0.0, material.metallic), 1.0, matte);
...
fn evaluate_diffuse_brdf(wo: vec3<f32>, wi: vec3<f32>, world_normal: vec3<f32>, material: ResolvedMaterial) -> vec3<f32> {
    ...
    if material.reflectance == 0.0 && material.metallic == 0.0 {
        return diffuse_color * NdotL;
    }
    let F0 = calculate_F0(material.base_color, material.metallic, vec3(material.reflectance));
