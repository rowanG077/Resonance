...
fn shared_data_keeps_source_identity_and_rejects_wrong_links() -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        let paths = write(&file, &output, "tables", &vec![1u8, 2])?;
        assert_eq!(read::<Vec<u8>>(&output, "tables", "source.rel")?, [1, 2]);
        assert_eq!(
            serde_json::from_slice::<serde_json::Value>(&fs::read(output.join(&paths[1]))?)?,
            json!({"module":"source.rel", "source_sha256":crate::digest(b"source"), "data":paths[0]})
        );
