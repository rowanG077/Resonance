...
use resonance_audio_cook::{
    bank::{Bank, MissingObject, ObjectKind, Page, Sound},
...
impl Cooker {
    pub(crate) fn new(
        ...
        Ok(Self {
            workspace: Workspace::open(extracted, output)?,
            executable,
            coefficients,
            song_setups,
            pools,
    ...
    pub(crate) fn cook_song(&self, source: &str) -> Result<MemberResults> {
        ...
        let ids = self
            ...
            .unwrap_or(&[]);
        let bank = (|| -> Result<_> {
            let mut bank = Bank::parse(&self.pools.instruments)?;
            self.pools.inherit(&mut bank)?;
            Ok(bank)
        })();
                    ...
                    match event.kind {
                        ...
                        EventKind::Note { key, velocity, .. } => {
                            if let Some(page) = setup.page(event.channel, programs[channel]) {
                                ...
                                notes.push(json!({"event":index,"voices":voices}));
                            }
                        }
                        _ => {}
                    }
    ...
    pub(crate) fn archive_members(
        ...
        let mut index = VoiceArchive {
            version: VoiceArchive::VERSION,
            source_sha256: sha256.into(),
            members: Vec::with_capacity(entries.len()),
        };
        for (id, entry) in entries.iter().enumerate() {
            ...
            let empty =
                entry.size >= 16 && is_stream(&header) && crate::read::u32(&header, 12)? == 0;
            index.members.push(Member {
                name: entry.name.clone(),
                metadata: format!("{directory}/{id}.json"),
                kind: if empty {
                    MemberKind::Empty
                } else {
                    MemberKind::Stream
...
impl Pools {
    fn read(extracted: &Path) -> Result<Self> {
        let executable = fs::read(extracted.join("sys/main.dol"))?;
    ...
    for id in sounds {
        ...
        let result = (|| {
            ...
            resource_file(
                ...
                &path,
                resources,
                Some(media::sound_score(id, Some(notes))),
            )
        })();
        let result = match result {
            Err(error) if error.downcast_ref::<MissingObject>().is_some() => {
                let dependency = *error.downcast_ref::<MissingObject>().unwrap();
                pools.absent(&dependency).and_then(|absent| {
                    if absent {
...
/// Enemy tables and their separately stored sample payload form one sound bank.
/// Only this group's payload range is read from the voice-bank archive.
pub(crate) fn cook_embedded_bank(
    extracted: &Path,
    output: &Path,
    enemy: &[u8],
    pools: &Pools,
    ...
    let metadata = usize::from(half(enemy, 4)?);
    let group = usize::from(
        *enemy
            .get(metadata + 0xa9)
            .context("truncated enemy sound group")?,
    );
    ensure!(group >= 19, "invalid embedded sound group {group}");
    let source = crate::all_assets::voice_bank_path(extracted)?;
    let (mut archive, directory) = bank_archive(extracted, &source, &pools.sources()?.usual)?;
    let range = directory
        .get(group - 19..group - 17)
        .context("embedded sound group exceeds archive directory")?;
    let samples = payload(&mut archive, range[0], range[1])?;
    let bank = Bank::from_sections(embedded_sections(enemy, &samples)?)?;
    let directory = format!("audio/embedded/{}", crate::digest(enemy));
    cook_bank_data(output, &directory, bank, pools)
}
...
fn available_objects(pools: &Pools) -> Result<BTreeSet<(ObjectKind, u16)>> {
    ...
    let offsets = super::super::actions::member(&usual, 10)?
        .chunks_exact(4)
        .map(|bytes| crate::read::u32(bytes, 0))
        .collect::<Result<Vec<_>>>()?;
    let mut archive = fs::File::open(extracted.join("files").join(&sources.enemy))?;
    for (id, range) in super::super::all::physical_ranges(&offsets, 0, archive.metadata()?.len())? {
...
mod tests {
    ...
    fn original_voice_bank_members_follow_native_directory() -> Result<()> {
        for disc in [1, 2] {
            ...
            let usual = fs::read(extracted.join("files").join(&sources.usual))?;
            let authored = super::super::super::actions::member(&usual, 13)?;
    ...
    fn original_song_aliases_keep_all_setups_and_isolate_member_failures() -> Result<()> {
        ...
        for disc in [1, 2] {
            ...
            let files = fs::read_dir(extracted.join("files/S"))?
                ...
                .collect::<Result<BTreeMap<_, _>>>()?;
            let mut cooker = Cooker::new(&extracted, &output, &coefficients, &files)?;
