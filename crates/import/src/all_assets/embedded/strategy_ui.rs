...
fn parse(executable: &[u8]) -> Result<(Catalogue, Vec<TextSource>)> {
    ...
    let catalogue = Catalogue {
        ...
        keyboard: Keyboard {
            ...
            input_aliases_cells: input_address == KEYBOARD,
    ...
    fn original_strategy_ui_reconstructs_complete_tables_and_publishes_shared_data() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                assert!(c.keyboard.input_aliases_cells);
                ...
                assert!(!changed.keyboard.input_aliases_cells);
