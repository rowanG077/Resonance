...
const COMMON_LABELS: [CommonLabel; 41] = {
    use CommonLabel::*;
    [
        BuyCancel,
        Buy,
...
pub(super) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Vec<String>> {
    ...
    // The native default really reads beyond the 41-pointer table. Record its
    // unresolved target; do not turn the adjacent inline string into another row.
    let instruction = word(dol::slice(executable, 0x800b5c00, 4)?, 0)?;
    ensure!(
        instruction >> 16 == 0x8004,
        "unexpected save-menu default message lookup"
    );
    let default_offset = i32::from(instruction as i16);
    let target_cell = COMMON
        .checked_add_signed(default_offset)
    ...
    crate::embedded::write(
        ...
        &catalogue,
        serde_json::json!({
            "common":{"address":COMMON,"count":41,"stride":4,"source_size":164},
            "card_messages":{"address":MESSAGES,"count":57,"stride":4,"source_size":228,"shared_pointer_index":24},
            "choices":{"address":CHOICES,"count":2,"stride":4,"selectable_per_group":3,"source_size":8},
            "probe_file":{"address":0x8035cdbcu32,"source_size":4},
            "text_addresses":addresses,
            "default_error_reference":{"instruction":0x800b5c00u32,"byte_offset":default_offset,"outside_common_table":!(0..164).contains(&default_offset),"target_address":word(dol::slice(executable,target_cell,4)?,0)?}
        }),
    )
}
...
mod tests {
    ...
    fn original_save_menu_preserves_pointer_aliases_choices_and_all_messages() -> Result<()> {
        ...
        for disc in [1, 2] {
            ...
            assert_eq!(
                word(dol::slice(&executable, 0x800b5c00, 4)?, 0)?,
                0x800400a4
            );
            assert_eq!(
                word(dol::slice(&executable, COMMON + 164, 4)?, 0)?,
                0x43686563
            );
