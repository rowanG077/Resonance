...
fn parse(executable: &[u8]) -> Result<(Catalogue, Vec<TextSource>)> {
    ...
    let characters: Vec<_> = dol::slice(executable, COMPOUNDS, 9 * 196)?
        ...
        .map(|(row, choices)| {
            let compounds: Vec<_> = row[4..]
                .chunks_exact(8)
                .map(|record| {
                    Ok(Compound {
                        skill: half(record, 0)?,
                        requirement_count: half(record, 2)?,
                        requirements: record[4..8].try_into()?,
                    })
                })
                .collect::<Result<_>>()?;
            Ok(Character {
                ...
                compounds: compounds.try_into().unwrap(),
            })
        })
        .collect::<Result<_>>()?;
    let personal = dol::slice(executable, PERSONAL_SKILLS, 24)?;
    let mut personal_skills = [0; 9];
    for (i, skill) in personal_skills.iter_mut().enumerate() {
        *skill = half(personal, i * 2)?;
    }
...
pub(super) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Vec<String>> {
    ...
    crate::embedded::write(
        ...
        &catalogue,
        serde_json::json!({
            "definitions":{"address":DEFINITIONS,"count":DEFINITION_COUNT,"stride":20,"source_size":3324,
                "uninterpreted_storage":{"offset":3320,"source_size":4}},
            "choices":{"address":CHOICES,"count":9,"stride":16,"source_size":144},
            "compounds":{"address":COMPOUNDS,"count":9,"stride":196,"source_size":1764},
            "personal_skills":{"address":PERSONAL_SKILLS,"count":9,"stride":2,"source_size":24,
                "uninterpreted_storage":{"offset":18,"source_size":6}},
            "labels":{"address":LABELS,"count":19,"stride":4,"source_size":80,
                "uninterpreted_storage":{"offset":76,"source_size":4}},
            "formats":{"address":0x8035d648u32,"source_size":40,"slot_sizes":[8,8,4,8,4,8]},
            "save_point_rule":{"character_index":{"address":0x800cf788u32,"source_size":4},
                "tp_cost":{"address":0x800cf7a0u32,"source_size":4}},
            "texts":sources,
        }),
    )
}
    ...
    fn original_ex_catalogue_preserves_definitions_recipes_aliases_and_storage() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                assert_eq!(provenance["definitions"]["source_size"], 3324);
                assert_eq!(provenance["compounds"]["source_size"], 1764);
