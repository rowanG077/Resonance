...
pub(super) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Vec<String>> {
    ...
    crate::embedded::write(
        ...
        &catalogue,
        serde_json::json!({
            "labels":{"address":TABLE,"count":LABELS.len(),"stride":4,"source_size":LABELS.len()*4},
            "texts":texts,
        }),
    )
}
    ...
    fn original_shop_ui_preserves_all_bindings_aliases_and_source_texts() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                assert_eq!(provenance["labels"]["count"], 26);
                assert_eq!(provenance["labels"]["source_size"], 104);
