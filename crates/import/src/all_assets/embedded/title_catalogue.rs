...
fn parse(executable: &[u8]) -> Result<(Catalogue, Vec<TextSource>)> {
    ...
    let mut character_starts = [0; 9];
    for (index, start) in character_starts.iter_mut().enumerate() {
        *start = half(starts, index * 2)?;
    }
    let rows = dol::slice(executable, ENTRIES, COUNT * STRIDE + 4)?;
    let costume_titles = dol::slice(executable, COSTUME_TITLES, 56)?;
    let mut titles = [[0; 9]; 3];
    for (title, bytes) in titles
        .iter_mut()
        .flatten()
        .zip(costume_titles.chunks_exact(2))
    {
        *title = i16::from_be_bytes(bytes.try_into()?);
    }
...
pub(super) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Vec<String>> {
    ...
    crate::embedded::write(
        ...
        &catalogue,
        serde_json::json!({
            "character_starts":{"address":STARTS,"count":9,"stride":2,"source_size":20,
                "uninterpreted_storage":{"offset":18,"source_size":2}},
            "entries":{"address":ENTRIES,"count":COUNT,"stride":STRIDE,"source_size":COUNT*STRIDE+4,
                "uninterpreted_storage":{"offset":COUNT*STRIDE,"source_size":4}},
            "costume_titles":{"address":COSTUME_TITLES,"count":3,"stride":18,"characters_per_row":9,"source_size":56,
                "uninterpreted_storage":{"offset":54,"source_size":2}},
            "costume_variants":{"address":COSTUME_VARIANTS,"count":3,"stride":1,"source_size":4,
                "uninterpreted_storage":{"offset":3,"source_size":1}},
            "texts":sources,
        }),
    )
}
    ...
    fn original_title_catalogue_preserves_all_records_ranges_and_storage() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                assert_eq!(provenance["entries"]["source_size"], 2548);
                assert_eq!(provenance["costume_titles"]["source_size"], 56);
                assert_eq!(provenance["costume_variants"]["source_size"], 4);
