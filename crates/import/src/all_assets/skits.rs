...
impl Catalog {
    ...
    pub(crate) fn definitions(&self) -> Result<Vec<SkitDefinition>> {
    ...
    pub(crate) fn script(&self, id: u16) -> Result<&str> {
        let binding = self
            ...
            .with_context(|| format!("unknown skit {id}"))?
            .script
            .binding
            .context("skit has no filename slot")?;
        let file = self
            .physical
            .scripts
            .iter()
            .find(|script| script.binding == binding)
            .context("missing skit filename binding")?
            .file
            .context("null skit filename")?;
        self.physical
            .texts
            .get(file.0)
            .map(String::as_str)
            .context("invalid skit filename reference")
    }
}
...
impl Physical {
    fn prepared(&self) -> Result<Vec<SkitDefinition>> {
        ...
        for (group, first, count) in [(Group::Story, 1, 119), (Group::Timed, 600, 259)] {
            ...
            ensure!(rows.clone().count() >= count, "truncated skit definitions");
            for (index, row) in rows.take(count).enumerate() {
                ensure!(
                    usize::from(row.id) == first + index,
                    "invalid skit table order"
                );
                let story = match row.story {
                    Story::Disabled => continue,
                    Story::Any => None,
                    Story::Range { first, last } => Some([first, last]),
                };
                let title = self
                    .texts
                    .get(row.title.context("null skit title")?.0)
                    .context("invalid skit title reference")?
                    .clone();
                let location = match row.location {
                    Location::Any => SkitLocation::Anywhere,
                    Location::Overworld => SkitLocation::Overworld(None),
                    Location::Sylvarant => SkitLocation::Overworld(Some(0)),
                    Location::TetheAlla => SkitLocation::Overworld(Some(1)),
                    Location::Field => SkitLocation::Field,
                    Location::Map { id } => SkitLocation::Map(id),
                    Location::Unknown { value } => anyhow::bail!("unknown skit location {value}"),
                };
...
pub(crate) struct Portrait {
    ...
    pub(crate) images: Vec<PortraitImage>,
}
...
pub(crate) fn cook(extracted: &Path, output: &Path) -> Result<Vec<String>> {
    ...
    crate::embedded::write(&file, output, "skits", &catalog)
}
    ...
    fn original_skit_records_preserve_complete_fields_pointers_and_publication() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                let restored: Catalog = serde_json::from_value(published)?;
                let restored = restored.physical;
