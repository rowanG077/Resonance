...
use crate::{
    battle::audio::all as audio, compression, field::sections, read::u32 as word, write_atomic,
};
...
/// Package headers can identify a secondary model's shared primary texture table.
pub(crate) fn cook_with_palette<F: FnMut(&str, Result<()>)>(
    bytes: &[u8],
    primary: &[u8],
    name: &str,
    output: &Path,
    ...
    .visit(bytes, name, Some(primary), 0, Input::File)
}
...
impl<F: FnMut(&str, Result<()>)> Walker<'_, '_, F> {
    fn visit(
        ...
        if matches!(word(bytes, 0x20).ok(), Some(0x005b_bc61 | 0x00b7_49e0)) {
        ...
        match word(bytes, 0).ok() {
            ...
            Some(0x007b_7960) => {
                let result = if crate::animation::is_animation(bytes) {
                    ...
                    match input {
                        Input::File => crate::animation::unbound(bytes),
                        Input::Member => crate::animation::unbound_indexed(bytes),
                        Input::Field => unreachable!("field inputs were dispatched above"),
                    }
                    .and_then(|motion| self.json(name, "animation", &motion))
                } else {
        ...
        if let Ok(ranges) = sections(bytes) {
            ...
            let attachment = (5..=7).contains(&ranges.len())
                ...
                && ranges[1].as_ref().is_some_and(|r| {
                    matches!(
                        word(&bytes[r.clone()], 0x20).ok(),
                        Some(0x005b_bc61 | 0x00b7_49e0)
                    )
                });
            ...
            let mut primaries = ranges.iter().flatten().filter_map(|range| {
                ...
                (word(part, 0).is_ok_and(|offset| offset > 0)
                    && matches!(word(part, 0x20).ok(), Some(0x005b_bc61 | 0x00b7_49e0)))
                .then_some(part)
            });
            ...
            for (index, range) in ranges.iter().enumerate() {
                if let Some(range) = range {
                    if (actor
                        && matches!(index, 29 | 30)
                        && !crate::animation::is_animation(&bytes[range.clone()]))
                        || (terrain && index == 4)
                    {
                    ...
                    if index == 0 && attachment {
                        let child = format!("{name}/{index}");
                        let result = crate::battle::all::attachment_recipe(&bytes[range.clone()])
        ...
        // Action records have no magic. Their rule-table base distinguishes the
        // four-phase header from retained compact records after archive detection.
        let phases = word(bytes, 0).ok() == Some(128)
            && (1..4).all(|index| {
                word(bytes, index * 4).is_ok_and(|base| base >= 128 && base as usize <= bytes.len())
            });
        let compact = word(bytes, 12).ok() == Some(28)
            && (16..28).step_by(4).all(|at| {
                word(bytes, at).is_ok_and(|base| base >= 28 && base as usize <= bytes.len())
            });
        if phases || compact {
            let result = crate::battle::physical_bundle(bytes)
                .and_then(|bundle| self.json(name, "actions", &bundle));
            (self.report)(name, result);
            return true;
        }
        false
    ...
    fn model(&mut self, bytes: &[u8], name: &str, palette: Option<&[u8]>) -> Result<()> {
        ...
        let scene = super::physical_scene::cook(&normalized, name, self.output, textures)?;
    ...
    fn skeleton(&self, bytes: &[u8], name: &str) -> Result<()> {
        ...
        self.json(name, "skeleton", &skeleton_nodes(&model)?)?;
        self.json(
            name,
            "motion-bindings",
    ...
    fn textures(&mut self, bytes: &[u8], name: &str) -> Result<()> {
        use crate::texture::{Catalogue, Texture};
        ...
        for (index, texture) in crate::tpl::parse_tpl(bytes)?.into_iter().enumerate() {
            ...
            let (mut metadata, levels) = match metadata {
                Ok(value) => value,
                Err(error) => {
                    (self.report)(&format!("{name}/texture-{index}"), Err(error));
            ...
            for page in 0..pages {
                ...
                metadata.images.push(format!("{key}.ktx2"));
                let result = (|| -> Result<()> {
                    let mut pngs = Vec::new();
                    for (level, mut image) in levels.iter().cloned().enumerate() {
                        if stride != 0 {
                        ...
                        let rgba = crate::tpl::decode_texture(bytes, &image)?;
                        let png = crate::temporary_path(
                            &self
                                .output
                                .join(format!("intermediate/{key}-mip-{level}.png")),
                        );
                        fs::create_dir_all(
                            png.parent().context("texture intermediate directory")?,
                        )?;
                        image::save_buffer(
                            &png,
                            &rgba,
                    ...
                    let destination = self.output.join(format!("{key}.ktx2"));
                    fs::create_dir_all(
                        destination
                            .parent()
                            .context("texture destination directory")?,
                    )?;
                    crate::texture::cook_levels(&pngs, &destination)?;
                    for png in pngs {
                        fs::remove_file(png)?;
                    }
                    Ok(())
...
fn compressed(bytes: &[u8]) -> Option<&[u8]> {
...
mod tests {
    ...
    fn cabinet_members_dispatch_by_structure_and_preserve_declared_field_roles() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            let restored: serde_json::Value = serde_json::from_slice(&fs::read(
                output.join("actions/retained.record/actions.json"),
            )?)?;
            assert_eq!(restored["phases"].as_array().unwrap().len(), 1);
            assert_eq!(
                restored["unused_storage"],
                serde_json::json!({"offset":28,"bytes":[0,0,0,0]})
            );
