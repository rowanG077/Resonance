...
fn vector<const N: usize>(bytes: &[u8], offset: usize) -> Result<[f32; N]> {
    let mut result = [0.; N];
    for (index, value) in result.iter_mut().enumerate() {
        *value = float(bytes, offset + index * 4)?;
    }
    Ok(result)
}
...
pub(crate) fn camera(bytes: &[u8]) -> Result<CameraTrack> {
    ...
    let transforms = (0..count)
        .map(|index| {
            let offset = 12 + index * 36;
            Ok(TransformKey {
                time: float(bytes, offset)?,
                position: vector(bytes, offset + 4)?,
                rotation: vector(bytes, offset + 16)?,
                unused_word: word(bytes, offset + 32)?,
            })
        })
    ...
    let targets = if target_offset == 0 {
        ...
        (0..count)
            .map(|index| {
                let offset = start + index * 20;
                Ok(PositionKey {
                    time: float(bytes, offset)?,
                    position: vector(bytes, offset + 4)?,
                    unused_word: word(bytes, offset + 16)?,
                })
            })
            .collect::<Result<Vec<_>>>()?
    };
