...
fn decode(
    ...
    mut name: impl FnMut(usize) -> Result<((usize, usize), String)>,
    ...
    rows.chunks_exact(ROW_BYTES)
        ...
        .map(|(index, row)| {
            ...
            let (pointer, name) = name(index)?;
            ensure!(!name.is_empty(), "empty long-range region name");
            let next = names.len();
            Ok(LongRangeUnlock {
                name,
                name_id: *names.entry(pointer).or_insert(next),
                location,
...
pub(crate) fn read(
    file: &Path,
    locations: &BTreeMap<u16, MapLocation>,
) -> Result<Option<Vec<LongRangeUnlock>>> {
...
pub(super) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Option<Vec<String>>> {
    ...
    let Some(records) = read(file, &world.locations)? else {
        return Ok(None);
    };
    embedded::write(file, output, "long-range-unlocks", &records).map(Some)
}
    ...
    fn original_long_range_unlocks_cover_all_field_modules() -> Result<()> {
        ...
        for disc in [1, 2] {
            ...
            for module in [
                ...
                assert!(decode(bytes, &world.locations, |_| Ok(((0, 0), String::new()))).is_err());
                let aliases = decode(bytes, &world.locations, |index| {
                    names(if index == 1 { 0 } else { index })
                })?;
                assert_eq!(aliases[0].name_id, aliases[1].name_id);
                assert_eq!(aliases[0].name, aliases[1].name);
            }
        }
        assert_eq!(
            publications.len(),
