...
pub(super) fn cook(file: &Path, output: &Path) -> Result<Option<Vec<String>>> {
    ...
    embedded::write(
        ...
        &tables,
        json!({
            "data_section": DATA, "rodata_section": RODATA, "layout": layout,
            "travel_points_offset": TRAVEL_POINTS_TABLE,
        }),
    )
    .map(Some)
}
...
fn original_overworld_encounters_cover_all_field_modules() -> Result<()> {
    ...
    for disc in [1, 2] {
        for module in [
            ...
            assert_eq!(provenance["layout"], serde_json::to_value(layout)?);
            assert_eq!(provenance["travel_points_offset"], TRAVEL_POINTS_TABLE);
