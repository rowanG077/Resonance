...
fn original_exclusions_are_metadata_or_native_containers_with_publications() -> Result<()> {
    use std::collections::BTreeMap;
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../local");
    let excluded: BTreeMap<String, String> =
        serde_json::from_slice(&fs::read(root.join("all-assets/excluded.json"))?)?;
    let sources: BTreeMap<String, Vec<String>> =
        serde_json::from_slice(&fs::read(root.join("all-assets/sources.json"))?)?;
    let mut counts = [0; 4];
    for (source, reason) in &excluded {
        ...
        match reason.as_str() {
            ...
            "native_code" => {
                ...
                // Publications prove these are not wholly skipped. Discovery of
                // every embedded table remains a separate consumer audit.
                let outputs = &sources[source];
                assert!(
                    !outputs.is_empty(),
                    "native container hides all data: {source}"
                );
                for output in outputs {
                    assert!(root.join("all-assets").join(output).exists(), "{output}");
                }
                counts[2] += 1;
            }
            "disc_metadata" => {
