...
pub(super) const BANKS: [(Bank, u32); 17] = [
    (Bank::Frames, 0x80234260),
    (Bank::Plain, 0x80231720),
    (Bank::Alternate, 0x80236e80),
    (Bank::Patterns, 0x80231960),
    (Bank::Cursor, 0x80249500),
    (Bank::Symbols, 0x80249780),
    (Bank::WorldMaps, 0x8024e2a0),
    (Bank::Portraits, 0x8023d3e0),
    (Bank::Technique, 0x8024ba00),
    (Bank::Strategy, 0x80241d80),
    (Bank::Elements, 0x8024ae20),
    (Bank::Numbers, 0x8026a1c0),
    (Bank::Items, 0x80243340),
    (Bank::ItemTabs, 0x80247d40),
    (Bank::Buttons, 0x80239380),
    (Bank::Recipes, 0x80212060),
    (Bank::Conditions, 0x80269380),
];

#[derive(Serialize, Deserialize)]
pub(super) struct Recipe {
    pub(super) banks: BTreeMap<Bank, String>,
...
pub(super) fn read(
    ...
    let mut recipe = Recipe {
        banks: BANKS
            .into_iter()
            .map(|(bank, address)| Ok((bank, dol::texture_bank(executable, address)?)))
            .collect::<Result<_>>()?,
        ...
        names: crate::character_data::read(executable)?
            .definitions
            .into_iter()
            .take(9)
            .map(|row| row.name)
