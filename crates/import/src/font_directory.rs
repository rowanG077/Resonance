...
impl Metrics {
    ...
    pub(crate) fn code(&self, character: char) -> Result<u16> {
        ...
        if character.is_ascii() {
            return Ok(self.ascii_codes[(character as usize).max(32) - 32]);
        }
        let character = character.to_string();
        let (bytes, _, invalid) = encoding_rs::SHIFT_JIS.encode(&character);
        ensure!(
            !invalid && bytes.len() == 2,
            "invalid font character {character:?}"
        );
        Ok(u16::from_be_bytes(bytes.as_ref().try_into()?))
    }
    ...
    pub fn cook(extracted: &Path, output: &Path) -> Result<Vec<String>> {
        ...
        embedded::write(
            ...
            serde_json::json!({
                ...
                "banks":{"address":BANKS,"count":COUNT,"stride":4},
                "palette":{"address":PALETTE,"count":COUNT,"stride":2,"format":"rgb5a3"},
                "metrics":{
                    "ascii_codes":{"address":ASCII_CODES,"count":96,"stride":2},
                    "alternate_codes":{"address":ALTERNATE_CODES,"count":66,"stride":2},
                    "advances":{"address":ADVANCES,"count":288,"stride":1}
                }
            }),
        )
    }
    ...
    fn original_font_directories_preserve_native_bindings_and_all_physical_banks() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            for disc in 1..=2 {
                ...
                assert_eq!(provenance["palette"]["count"], COUNT);
                for (name, count) in [
                    ("ascii_codes", 96),
                    ("alternate_codes", 66),
                    ("advances", 288),
                ] {
                    assert_eq!(provenance["metrics"][name]["count"], count);
                }
            }
            Ok(())
        })();
