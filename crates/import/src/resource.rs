...
pub(crate) fn cook(file: &Path, executable: &[u8], output: &Path) -> Result<Vec<String>> {
    embedded::write(
        ...
        &read(executable)?,
        serde_json::json!({
            "standalone":{"address":STANDALONE,"count":53,"stride":4},
            "groups":{"address":GROUPS,"count":14,"stride":12,"runtime_fields":[
                {"name":"loaded_directory","offset":4,"bytes":4},
                {"name":"valid","offset":8,"bytes":1}
            ]},
            "party_bodies":{"address":BODIES,"count":9,"stride":20},
            "party_battle_motions":{"address":BATTLE_MOTIONS,"count":12,"stride":20},
            "party_field_motions":{"address":FIELD_MOTIONS,"count":9,"stride":INLINE_BYTES},
            "field_services":{"address":FIELD_SERVICES,"count":10,"stride":16},
        }),
    )
}
...
mod tests {
    ...
    fn resource_catalogue_preserves_declarations_and_excludes_loader_state() -> Result<()> {
        ...
        executable[offsets[6]..].fill(b'a');
        assert!(read(&executable).is_err());
        executable[offsets[6]..].fill(0);
        for (index, stride) in [(4, INLINE_BYTES), (5, 16)] {
    ...
    fn original_resource_catalogue_reconstructs_and_publishes_both_discs() -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            for disc in [1, 2] {
                ...
                assert_eq!(
                    provenance["field_services"],
                    serde_json::json!({"address":FIELD_SERVICES,"count":10,"stride":16})
                );
                assert_eq!(
                    provenance["groups"]["runtime_fields"],
                    serde_json::json!([
                        {"name":"loaded_directory","offset":4,"bytes":4}, {"name":"valid","offset":8,"bytes":1}
                    ])
                );
