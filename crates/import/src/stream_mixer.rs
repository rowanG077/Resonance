...
impl Tables {
    ...
    pub fn cook(extracted: &Path, output: &Path) -> Result<Vec<String>> {
        ...
        embedded::write(
            ...
            serde_json::json!({
                "settings":{"address":SETTING_ADDRESS,"count":SETTING_COUNT,"storage_bytes":4},
                "volume":{"address":VOLUME_ADDRESS,"count":VOLUME_COUNT},
                "pan":{"address":PAN_ADDRESS,"count":PAN_COUNT,"stride":4,"storage_bytes":4},
            }),
        )
    }
}
