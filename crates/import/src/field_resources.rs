...
pub(crate) fn declarations(bytes: &[u8]) -> Result<Declarations> {
    ...
    for (pc, args) in arguments {
        if let Some(value) = args[0] {
            ...
            resources.extend(finite.get(&pc).with_context(|| {
    ...
    Ok(Declarations {
        ...
        save_point: analysis.instructions.values().any(|i| {
...
";
    ...
    fn finite_party_lookups_require_complete_paths_and_preserve_aliases() -> Result<()> {
        ...
        for helper in [false, true] {
            ...
            assert_eq!(declarations(&bytes)?.resources, expected);
            assert_eq!(
            ...
            for uncertain in [
                ...
                let bytes = scenario::assemble(&uncertain)?;
                assert!(declarations(&bytes).is_err(), "admitted uncertain lookup: {uncertain}");
            }
        }
        Ok(())
    }
