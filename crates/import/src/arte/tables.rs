...
pub(crate) struct LearningList {
    pub(crate) count: u8,
    /// Menu arte IDs; slot order also defines learned-state bit positions.
    pub(crate) technique_slots: Vec<u8>,
}

impl LearningList {
    pub(crate) fn active(&self) -> Result<&[u8]> {
        ensure!(
...
pub(crate) struct Combination {
    ...
    pub(crate) native_id: i16,
    pub(crate) participant_count: u16,
    /// Native arte IDs. A zero first ingredient disables that alternative.
    pub(crate) recipe_slots: [[i16; 4]; 6],
    pub(crate) duration_ticks: i16,
    pub(crate) storage: [u8; 2],
