...
impl Animation {
    /// Bind authored channels without changing their keys or interpolation.
    pub(crate) fn motion(&self, model: &super::ModelBindings) -> Result<super::Motion> {
        ...
        Ok(super::Motion {
            ...
            tracks: model
                ...
                .into_iter()
                .map(|(bone, track)| {
                    let mut pose = self.tracks[track].pose(bone)?;
                    pose.bind_channels = *model
                        .channels
                        .get(usize::from(bone))
                        .context("missing model transform channels")?;
                    Ok(pose)
                })
                .collect::<Result<_>>()?,
        })
        ...
        for curve in &self.channels {
            ...
            let count = self.times.len();
                ...
                curve.values.len() == count
                    ...
                    && curve.ease.iter().flatten().all(|value| value.is_finite()),
                "invalid animation controls or time easing"
            );
            let flag = match curve.component {
                Component::Scale | Component::Translation | Component::EulerDegrees => {
                ...
                Component::Matrix3x4 => {
                        ...
                        "unsupported matrix interpolation"
                    );
                    track.matrices = Some(vectors(&curve.values)?);
                    16
                }
            };
...
pub(crate) fn decode(bytes: &[u8]) -> Result<Animation> {
...
mod tests {
    ...
    #[test]
    fn cooked_curves_preserve_binding_and_time_controls_after_json_roundtrip() -> Result<()> {
        ...
        let motion = decode(&bytes)?.motion(&bindings)?;
        assert_eq!(
            motion.tracks[0].translation.as_ref().unwrap().ease,
            [[1. / 16384., 0.]]
        );
        assert_eq!(
            serde_json::to_value(&motion)?,
            serde_json::to_value(super::super::motion(&bytes, &model)?)?
        );
        Ok(())
    }
    ...
    fn authored_channels_keep_all_scalar_widths_and_fractional_scales() {
        let bindings = super::super::ModelBindings {
            node_ids: vec![9],
            channels: vec![resonance_content::animation::TransformChannels(13)],
            names: None,
        };
        for (format, encoded, expected) in [
            ...
            assert_eq!(motion.tracks[0].bind_channels.0, 13);
            assert_eq!(
                motion.tracks[0]
                    .sample(0., resonance_content::animation::Transform::default())
                    .unwrap()
                    .translation
                    .as_slice(),
                expected.as_slice()
    ...
    fn float_vectors_and_quaternions_keep_independent_controls_and_ease() {
        ...
        assert_eq!(track.rotation.as_ref().unwrap().ease, rotation.ease);
        let curve = track.translation.as_ref().unwrap();
    ...
    fn original_float_field_animation_cooks_all_72_tracks() {
        ...
        let bindings = super::super::ModelBindings {
            ...
            channels: vec![Default::default(); animation.tracks.len()],
            names: None,
        };
