//! Cook title resource declarations once; preparation needs only the shared library.
...
impl Resource {
    ...
    pub fn bind<'a>(&self, output: &'a Path, disc: u8) -> Result<crate::cooked::Source<'a>> {
        crate::cooked::Source::open(output, disc, &self.path)?
            .published_directory(&format!("assets/{}", self.sha256))
    }
}
...
impl Recipe {
    ...
    pub fn bind(output: &Path, disc: u8) -> Result<Self> {
        ensure!((1..=2).contains(&disc), "invalid title source disc");
