...
impl<'a> Source<'a> {
    ...
    /// Follow a shared table's checked link within this source's publications.
    pub(crate) fn embedded<T: serde::de::DeserializeOwned>(
        &self,
        family: &str,
        module: &str,
    ) -> Result<T> {
        crate::embedded::read_with(family, module, |path| self.document_bytes(path))
    }
    ...
    /// Select a directory publication before resolving its child documents.
    pub(crate) fn published_directory(&self, suffix: &str) -> Result<Self> {
        validate_asset_path(suffix)?;
        let paths: Vec<_> = self
            ...
            .cloned()
            .collect();
        ensure!(!paths.is_empty(), "missing cooked {suffix}; rerun cook-all");
        Ok(Self {
            root: self.root,
            paths,
        })
