...
pub(crate) fn command(bank: &Bank<'_>, a: u32, b: u32) -> Result<Command> {
    ...
    Ok(match a as u8 {
        ...
        0x46 | 0x4b | 0x4c => {
                ...
                "only zero-scale Set variable selectors are implemented"
            );
        ...
        0x04 | 0x07 => {
            ensure!((a >> 16) as u8 <= 1, "unsupported random-wait flags");
            if (a >> 16) & 1 != 0 {
                ensure!(
                    (a as u8 == 0x07 || (b >> 8) & 1 != 0) && b & 1 == 0 && (a >> 24) & 1 == 0,
                    "only relative millisecond random waits without sample-end wake are implemented"
                );
                Command::RandomWait {
                    upper_ms: (b >> 16) as u16,
                    key_off: (a >> 8) & 1 != 0,
                }
            } else if a as u8 == 0x04 && (b >> 8) & 1 == 0 {
                ensure!(
                    b & 1 == 0,
                    "absolute beat waits require the global synthesizer clock"
                );
                Command::BeatWait {
                    ticks: ((b >> 16) != 65535).then_some((b >> 16) as u16),
                    key_off: (a >> 8) & 1 != 0,
                    sample_end: (a >> 24) & 1 != 0,
                }
            } else {
