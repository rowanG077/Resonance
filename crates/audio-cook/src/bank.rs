...
mod tests {
    ...
    #[test]
    fn variable_selectors_preserve_mixer_targets() {
        use resonance_audio::data::{Command, ControlTarget, Operand};
        for (opcode, expected_target) in [
    ...
    fn beat_wait_and_second_sweep_preserve_operands_and_reject_unimplemented_modes() {
        ...
        for upper in [0, 2000, 65535] {
            assert!(matches!(compile(0x00010107, upper << 16).unwrap(),
                Command::RandomWait { upper_ms, key_off: true, sample_end: false } if u32::from(upper_ms) == upper));
        }
        assert!(matches!(
