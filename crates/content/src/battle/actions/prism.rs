//! Six sword impacts and a finisher share one captured ground point and heading.
...
pub struct PrismRecipe {
    pub lifetime: u16,
    pub origin: GroundSpellOrigin,
    pub presentation: StoredSpellPresentation,
    /// Radians added once to the captured visual heading. Projectiles use the live caster heading.
    pub heading_offset: f32,
    pub effect_scale: f32,
    pub bursts: [PrismBurst; 7],
    pub rules: [HitRule; 2],
}
...
impl PrismRecipe {
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            "invalid Prism Sword origin, presentation or bursts"
        );
        for rule in self.rules {
            ensure!(
                matches!(rule.element, HitElement::Element(Element::Light)),
                "invalid Prism Sword rule element"
            );
            rule.impact_program_from(EffectBank::Techniques, Some(32))?;
        }
        Ok(())
    }
}
